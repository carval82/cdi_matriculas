<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar: {{ $estudiante->nombre_completo }}</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('estudiantes.update', $estudiante) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center gap-4 mt-6">
                        ...
                        <a href="{{ route('estudiantes.show', $estudiante) }}" class="text-sm text-gray-600 hover:underline">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
